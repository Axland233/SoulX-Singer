...
IDLE_UNLOAD_SECONDS = int(os.environ.get("SOULX_IDLE_UNLOAD_SECONDS", "300"))
...
class AppState:
    def __init__(self, use_fp16: bool = False) -> None:
        ...
        self.svs_config = load_config("soulxsinger/config/soulxsinger.yaml")
        self.preprocess_pipeline = None
        self.svs_model = None
        self.phoneset_path = "soulxsinger/utils/phoneme/phone_set.json"
        self.midi_parser = None
    ...
    def _schedule_idle_unload_locked(self) -> None:
        self._cancel_idle_unload()
        self._idle_timer = threading.Timer(IDLE_UNLOAD_SECONDS, self._unload_if_still_idle)
        self._idle_timer.daemon = True
        self._idle_timer.start()

    def _unload_if_still_idle(self) -> None:
        with self._idle_lock:
            if self._active_jobs != 0:
                return
            self._idle_timer = None
            self._unload_all_locked()
            print(f"[idle_unload] Released models and CUDA cache after {IDLE_UNLOAD_SECONDS} seconds idle.", flush=True)

    def _unload_all_locked(self) -> None:
        self._unload_preprocess_pipeline()
        self._unload_svs_model()
        self._unload_midi_parser()
        self._clear_cuda_cache()

    def _unload_preprocess_pipeline(self) -> None:
        if self.preprocess_pipeline is not None:
            self.preprocess_pipeline = None
            self._clear_cuda_cache()

    def _unload_svs_model(self) -> None:
        if self.svs_model is not None:
            self.svs_model = None
            self._clear_cuda_cache()

    def _unload_midi_parser(self) -> None:
        if self.midi_parser is not None:
            self.midi_parser = None
            self._clear_cuda_cache()

    def _ensure_preprocess_pipeline(self) -> None:
        self._unload_svs_model()
    ...
    def _ensure_svs_model(self) -> None:
        self._unload_preprocess_pipeline()
        self._unload_midi_parser()
        if self.svs_model is None:
    ...
    def _ensure_midi_parser(self) -> None:
        self._unload_svs_model()
        self._unload_preprocess_pipeline()
        if self.midi_parser is None:
    ...
    def run_preprocess(
        ...
        self._mark_busy()
        try:
            self._ensure_preprocess_pipeline()
    ...
    def meta2midi(self, meta_path: Path, midi_path: Path) -> None:
        self._mark_busy()
        try:
            self._ensure_midi_parser()
            self.midi_parser.meta2midi(meta_path, midi_path)
        finally:
            self._mark_idle()

    def midi2meta(self, midi, meta, audio, language: str = "Mandarin") -> None:
        self._mark_busy()
        try:
            self._ensure_midi_parser()
            self.midi_parser.midi2meta(midi, meta, audio, language=language)
        finally:
            self._mark_idle()
...
def _edit_metadata(
    ...
    try:
        ...
        APP_STATE.midi2meta(midi, meta, audio, language=language)
        return meta
    except Exception:
...
def _transcribe_prompt(
    ...
    try:
        ...
        if prompt_meta_path.exists():
            APP_STATE.meta2midi(prompt_meta_path, prompt_midi_path)
...
def _transcribe_target(
    ...
    try:
        ...
        if target_meta_path.exists():
            APP_STATE.meta2midi(target_meta_path, target_midi_path)
