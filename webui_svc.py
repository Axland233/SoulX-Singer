...
class AppState:
	def __init__(self, use_fp16: bool = False) -> None:
		...
		self.preprocess_device = self.device
		self.svc_model = None
	...
	def _unload_svc_model(self) -> None:
		if self.svc_model is not None:
			self.svc_model = None
			self._clear_cuda_cache()

	def _ensure_svc_model(self) -> None:
		self._unload_preprocess_pipeline()
	...
	def run_preprocess(self, audio_path: Path, save_path: Path, vocal_sep: bool) -> tuple[bool, str, Path | None, Path | None]:
		...
		try:
			...
			try:
				...
				_print_exception("run_preprocess_cuda_oom")
				self.preprocess_device = "cpu"
				self._unload_preprocess_pipeline()
				self._ensure_preprocess_pipeline()
				self.preprocess_pipeline.save_dir = str(save_path)
				self.preprocess_pipeline.run(
			...
			_print_exception("run_preprocess")
			self._clear_cuda_cache()
